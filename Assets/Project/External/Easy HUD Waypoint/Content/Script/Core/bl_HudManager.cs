...
public class bl_HudManager : MonoBehaviour {
    ...
    static bl_HudManager _instance;

    public static bl_HudManager instance {
        get {
            if (_instance == null) {
                _instance = FindObjectOfType<bl_HudManager> ();
            }
            return _instance;
        }
    }
    ...
    /// </summary>
    void OnGUI () {
        // if (bl_HudUtility.mCamera == null) return;
    ...
    void OffScreen (int i) {
        if (Huds[i].Arrow.ShowArrow) {
            ...
            if (!bl_HudUtility.isOnScreen (bl_HudUtility.ScreenPosition (Huds[i].m_Target), Huds[i].m_Target)) {
                ...
                float mRot = bl_HudUtility.GetRotation (bl_HudUtility.mCamera.pixelWidth / (2f), bl_HudUtility.mCamera.pixelHeight / (2f), Xpos, Ypos);
                //Get pivot from area
                ...
                GUI.DrawTexture (bl_HudUtility.ScalerRect (new Rect (mPivot.x + marge.x, (mPivot.y + ((!Huds[i].ShowDistance) ? 10 : 20)) + marge.y, 100, 100)), Huds[i].m_Icon);
                */
            }
