...
namespace VIRTUE {
    public class MyCustomRagdollBuilder : ScriptableWizard {
        ...
        string CheckConsistency () {
            ...
            foreach (BoneInfo bone in bones) {
                if (bone.anchor == null) return String.Format ("{0} has not been assigned yet.\n", bone.name);
            }
            return "";
        }
        ...
        [MenuItem ("GameObject/3D Object/My Custom Ragdoll Builder...", false, 2000)]
        static void CreateWizard () {
            DisplayWizard<MyCustomRagdollBuilder> ("Create Ragdoll");
        }
        ...
        void CalculateAxes () {
            if (head != null && pelvis != null) up = CalculateDirectionAxis (pelvis.InverseTransformPoint (head.position));
        ...
        void OnWizardUpdate () {
            if (root) {
                Transform[] bones = root.GetComponentsInChildren<Transform> ().Where (x => x.name.ToLower ().Contains ("mixamorig:")).ToArray ();
                pelvis = Array.Find (bones, x => x.name.Equals ("mixamorig:Hips"));
                leftHips = Array.Find (bones, x => x.name.Equals ("mixamorig:LeftUpLeg"));
                leftKnee = Array.Find (bones, x => x.name.Equals ("mixamorig:LeftLeg"));
                leftFoot = Array.Find (bones, x => x.name.Equals ("mixamorig:LeftFoot"));
                rightHips = Array.Find (bones, x => x.name.Equals ("mixamorig:RightUpLeg"));
                rightKnee = Array.Find (bones, x => x.name.Equals ("mixamorig:RightLeg"));
                rightFoot = Array.Find (bones, x => x.name.Equals ("mixamorig:RightFoot"));
                leftArm = Array.Find (bones, x => x.name.Equals ("mixamorig:LeftArm"));
                leftElbow = Array.Find (bones, x => x.name.Equals ("mixamorig:LeftForeArm"));
                rightArm = Array.Find (bones, x => x.name.Equals ("mixamorig:RightArm"));
                rightElbow = Array.Find (bones, x => x.name.Equals ("mixamorig:RightForeArm"));
                middleSpine = Array.Find (bones, x => x.name.Equals ("mixamorig:Spine"));
                head = Array.Find (bones, x => x.name.Equals ("mixamorig:Head"));
            }
        ...
        void AddJoint (string name, Transform anchor, string parent, Vector3 worldTwistAxis, Vector3 worldSwingAxis, float minLimit, float maxLimit, float swingLimit, Type colliderType, float radiusScale, float density) {
            ...
            else if (name.StartsWith ("Right")) bone.parent = FindBone ("Right " + parent);
        ...
        void Cleanup () {
            foreach (BoneInfo bone in bones) {
                ...
                foreach (Joint joint in joints) Undo.DestroyObjectImmediate (joint);
                Component[] bodies = bone.anchor.GetComponentsInChildren (typeof (Rigidbody));
                foreach (Rigidbody body in bodies) Undo.DestroyObjectImmediate (body);
                Component[] colliders = bone.anchor.GetComponentsInChildren (typeof (Collider));
                foreach (Collider collider in colliders) Undo.DestroyObjectImmediate (collider);
            }
        }
